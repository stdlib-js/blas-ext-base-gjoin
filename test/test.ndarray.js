...
tape( 'the function has an arity of 5', function test( t ) {
	t.strictEqual( gjoin.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a string created by joining strided array elements', function test( t ) {
	...
	actual = gjoin( x.length-1, '-', x, 1, 1 );
	t.strictEqual( actual, '2-3-4-5-6', 'returns expected value' );

	actual = gjoin( x.length-2, '|', x, 1, 2 );
	t.strictEqual( actual, '3|4|5|6', 'returns expected value' );

	actual = gjoin( 3, '-', x, 2, 0 );
	t.strictEqual( actual, '1-3-5', 'returns expected value' );

	// Negative stride...
	actual = gjoin( x.length, ',', x, -1, x.length-1 );
	t.strictEqual( actual, '6,5,4,3,2,1', 'returns expected value' );

	actual = gjoin( 3, '-', x, -2, x.length-1 );
	t.strictEqual( actual, '6-4-2', 'returns expected value' );
...
tape( 'the function returns an empty string if provided an `N` parameter less than or equal to zero', function test( t ) {
	...
	actual = gjoin( 0, ',', [ 1.0, 2.0, 3.0 ], 1, 0 );
	t.strictEqual( actual, '', 'returns expected value' );

	actual = gjoin( -1, ',', [ 1.0, 2.0, 3.0 ], 1, 0 );
	t.strictEqual( actual, '', 'returns expected value' );
...
tape( 'the function returns an empty string if provided an `N` parameter less than or equal to zero (accessors)', function test( t ) {
	...
	actual = gjoin( 0, ',', toAccessorArray( [ 1.0, 2.0, 3.0 ] ), 1, 0 );
	t.strictEqual( actual, '', 'returns expected value' );

	actual = gjoin( -1, ',', toAccessorArray( [ 1.0, 2.0, 3.0 ] ), 1, 0 );
	t.strictEqual( actual, '', 'returns expected value' );
